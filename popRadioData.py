#from popRadioCharts import popRadioCharts
...
class popRadioData:
    def __init__(self, debug=False, minYear=1, maxYear=9999):
        ...
        self.basename  = "popradiotop20"
        #self.baseDir  = "/Volumes/Piggy/Charts/{0}".format(self.
        self.files     = []
        self.chartData = None
        #self.sc        = popRadioCharts()
    ...
    def findFiles(self):
        savedir = join(self.basedir, "data", "popradiotop20", "categories")
    ...
    def getPopRadioData(self, bsdata, debug=False):
        retval = {}
        for table in bsdata.findAll("table"):
            trs = table.findAll("tr")
            chartName = trs[0].text
            chartName = " ".join(chartName.split())
            #print(chartName)
            yearData      = trs[1]
            try:
                year = int(yearData.text.split()[0])
            except:
                year = None

            #print(year)
            retval["Chart"] = chartName
            retval["Year"]  = year
            retval["Songs"] = []
            for tr in table.findAll("tr"):
                ...
                if len(tds) == 2:
                    ...
                    if debug:
                        print(songData)
                    artistData = songData.find('b')
                    if artistData is None:
                        artistData = songData.find("strong")

                    if debug:
                        print(artistData.text)
                    try:
                        artistName = artistData.text
                    ...
                    if False:
                        if " f/" in artistName:
                            artistNames = artistName.split(" f/")
                        else:
                    ...
                    artistData.extract()
                    titleName = songData.text.strip()
                    retval["Songs"].append({"Artists": artistNames, "Title": titleName})
                    #print(artistNames,'\t',titleName)
    ...
    def parse(self):
        ...
        for i,ifile in enumerate(self.files):
            ...
            retval = self.getPopRadioData(bsdata)
            
            category  = retval["Chart"]
            year      = retval["Year"]
            songs     = retval["Songs"]            
            
            if data.get(category) is None:
                data[category] = {}
            data[category][year] = songs
    ...
    def saveChartData(self):
        for category,categoryData in self.chartData.items():
            saveDir = join(self.basedir, "data", "popradiotop20", "results")
            if not isDir(saveDir):
                raise ValueError("Could not find directory: {0}".format(savedir))
