...
class uDiscoverMusicData:
    ...
    def findFiles(self):
        savedir = join(self.basedir, "data", "udiscovermusic", "categories")
    ...
    def parse(self):
        ...
        data[category] = categoryData["Artists"]
        self.chartData = data
        self.saveChartData()
    ...
    def saveChartData(self):
        for category,categoryData in self.chartData.items():
            saveDir = join(self.basedir, "data", "udiscovermusic", "results")
            if not isDir(saveDir):
                raise ValueError("Could not find directory: {0}".format(savedir))
