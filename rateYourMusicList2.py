...
import re
from rateYourMusicList2Charts import rateYourMusicList2Charts
from langdetect import detect




class rateYourMusicList2Data:
    def __init__(self, debug=False, minYear=1, maxYear=9999):
        
        self.basedir  = "/Volumes/Piggy/Charts/"
        self.basename  = "RateYourMusicList2"
        #self.baseDir  = "/Volumes/Piggy/Charts/{0}".format(self.
        self.files     = {}
        self.chartData = None
        self.sc        = rateYourMusicList2Charts()
    ...
    def findFiles(self, pattern=None):
        savedir = join(self.basedir, "data", "rymlist2", "categories")
    ...
    def getSaveChartFilename(self, categoryName):
        savedir = join(self.basedir, "data", "rymlist2", "results")
        if not isDir(savedir):
            raise ValueError("Could not find directory: {0}".format(savedir))
